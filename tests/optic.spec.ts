...
describe('index (on array)', () => {
  ...
  const rprism = O.optic_<Source>().index(1)
  type Focus = string | undefined

  it('preview defined', () => {
    const result: Focus = O.preview(rprism)(source1)
    expect(result).toEqual('bar')
  })
  it('preview undefined', () => {
    const result: Focus = O.preview(rprism)(source2)
    expect(result).toBeUndefined()
  })

  it('set defined - monomorphic', () => {
    const result: Source = O.set(rprism)('UPDATED')(source1)
    expect(result).toEqual(['foo', 'UPDATED', 'baz', 'quux'])
  })
  it('set undefined - monomorphic', () => {
    const result: Source = O.set(rprism)('UPDATED')(source2)
    expect(result).toEqual(source2)
  })
  it('modify defined - monomorphic', () => {
    const result: Source = O.modify(rprism)(x => `${x} UPDATED`)(source1)
    expect(result).toEqual(['foo', 'bar UPDATED', 'baz', 'quux'])
  })
  it('modify undefined - monomorphic', () => {
    const result: Source = O.modify(rprism)(x => `${x} UPDATED`)(source2)
    expect(result).toEqual(source2)
  })

  type Target = Array<string | number>
  it('modify defined - polymorphic', () => {
    const result: Target = O.modify(rprism)(x => x.length)(source1)
    expect(result).toEqual(['foo', 3, 'baz', 'quux'])
  })
  it('modify undefined - polymorphic', () => {
    const result: Target = O.modify(rprism)(x => x.length)(source2)
    expect(result).toEqual(source2)
  })

  it('remove defined', () => {
    const result: Source = O.remove(rprism)(source1)
    expect(result).toEqual(['foo', 'baz', 'quux'])
  })

  it('remove undefined', () => {
    const result: Source = O.remove(rprism)(source2)
...
describe('index (on string)', () => {
  ...
  const rprism = O.optic_<string>().index(3)
  type Focus = string | undefined
